...
//   GST              5 s      5      accuracy (status)
//   GSA              5 s      5      DOP (status)
//   PQTMEPE          5 s      5      estimated error (status)
//   PQTMSVINSTATUS  10 s     10      survey-in Obs/MeanAcc (status)
//   GSV             10 s     10      satellites; bulky NMEA
inline bool applyBaseStatusRates(LC29H_GNSS& gnss) {
    bool ok = true;
    ok = gnss.setMessageRate("RMC", 1, 1) && ok;
    ok = gnss.setMessageRate("GGA", 1, 1) && ok;
    ok = gnss.setMessageRate("GST", 1, 5) && ok;
    ok = gnss.setMessageRate("GSA", 1, 5) && ok;
    ok = gnss.setMessageRate("PQTMEPE", 1, 5) && ok;
    ok = gnss.setMessageRate("PQTMSVINSTATUS", 1, 10) && ok;
    ok = gnss.setMessageRate("GSV", 1, 10) && ok;
    return ok;
}
